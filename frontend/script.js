document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(event) {
        ...
        const allocatedVolume = document.getElementById('allocated_volume').value;
        ...
        const payload = {
            ...
            allocated_volume: parseFloat(allocatedVolume)
        };
        ...
        if (data && data.body) {
            ...
            if (parsedData.messages && parsedData.table_data) {
                ...
                for (const row of parsedData.table_data.co2e_emissions) {
                    co2eEmissionsTableHtml += `<tr><td>${row.Metric}</td><td><b>${row.Value}</b></td></tr>`;
                }

...
def lambda_handler(event, context):
    ...
    try:
        ...
        instance_categories = instance_data.groupby('Family')['Instance type'].apply(list).to_dict()
        ...
        co2e_emissions_data = [
            {'Metric': 'Estimated CO2e Emissions:', 'Value': f'<b>{estimated_co2e:,.0f} gCO2e</b>'.replace(",", ".")}
        ]
